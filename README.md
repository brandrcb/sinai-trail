...
- **[The Exodus Trail — Classic](index.html)** — the current v0.5 game in the original 1985 Apple II green-screen look. This is the default.
- **[The Exodus Trail — Deluxe](deluxe.html)** — the same v0.5 game in the colour look, with parallax desert, day/night cycle and weather.
- **[Original v0.1](v1/index.html)** — the first release, preserved and playable exactly as it shipped (renamed only).
...
**Difficulty model (v0.3):** the household has a hidden hardship score `S.H` (0–139) updated daily Oregon-Trail-style — `H = 0.9·H + misery` — where misery is 6 for a day's march plus weather, pace (cloud −1 / lag 0 / ahead +4), rations, no water or food (+8, compounding), low Emunah, barefoot, and grumbling (+5 each). H maps to good/fair/poor/very poor and drives the daily illness roll (`0.2·(H/140)²`). Illness: −20 on onset, −4/day, cure 3 %/day walking, 15 % resting, +10 % if Emunah > 70, 0 while pushing ahead; someone below 50 HP does not survive a new illness. Water is one skin per person per day (cap 16 days, wells refill 4 days, springs fill). Score: 500/400/300/200 per survivor by health band × role, flags ±100. Difficulty (title menu) scales misery and illness ×0.8 / 1 / 1.3. `analysis/sim.py` is the Monte Carlo model used to tune it.
...
See [`docs/PLAN.md`](docs/PLAN.md) for the full design and roadmap, [`docs/NAMES.md`](docs/NAMES.md) for the names, and [`docs/difficulty-analysis.md`](docs/difficulty-analysis.md) for the difficulty tuning.
