...
SINAI.STOPS.push({
  ...
  async run(g) {
    ...
    if (c === 0) {
      ...
      await g.page("Your voice joins the roar. 'If we were honest,' says one teacher, 'I think we'd hear an awful lot of our own voices in these verses' (BEMA 20). Still — the LORD hears it, and He counts.\n\nGRUMBLES: " + n + " of 10.");
    } else if (c === 1) {
      g.trust(4);
...
SINAI.STOPS.push({
  ...
  async run(g) {
    ...
    await g.insight("Pawson · Unlocking the OT part 9",
      "'They lived on WHAT IS IT for forty years.' What's for breakfast? What is it. What's for lunch? What is it. Not again. Pawson figures the nation needed two million gallons of water a day. And a warning: 'Don't complain to God — He might give you what you want.'");
...
SINAI.STOPS.push({
  ...
  async run(g) {
    ...
    if (c === 0) { const n = g.grumble("Rephidim (Massah)"); g.trust(-4); await g.page("The place gets two names for what you just did: MASSAH (testing) and MERIBAH (quarreling).\n\n'The issue is not that they asked for water. The issue is that they demanded it, as if Yahweh didn't intend to provide.' (BibleProject)\n\nGRUMBLES: " + n + " of 10."); }
    else if (c === 1) { g.trust(5); await g.page("Asking is not testing. Gideon asked for a fleece and was not counted a rebel. 'Help me in my unbelief' is not the same as 'unless you do X, I won't move.' (BEMA 21) EMUNAH rises."); }
    ...
    if (arms === 0) { g.trust(5); g.flock(-2); await g.page("Two goats wander off while your sons hold up a tired old man's hands. 'It's about how we help other people.' (BEMA 21) That is the test of the MEOD — your 'very,' your resources, spent on someone else."); }
...
SINAI.STOPS.push({
  ...
  async run(g) {
    ...
    if (up === 0) await g.page("You stay below. Was that reverence, or a failure to accept the invitation? Two teachers disagree. Tim Mackie: refusing to go up is a failed test. Carmen Imes: staying below was the RIGHT answer — proper reverence. (BibleProject, 'Two Takes on the Test at Mount Sinai')\n\n'The Bible is designed to be meditation literature. We are not meant to understand everything on the first read.'");
    else { g.flag("went_up"); await g.page("You climb as far as the boundary and stand there while the mountain smokes and the shofar grows louder and louder. The people below have backed away. You have not. Two teachers disagree about whether that was right. (BibleProject, 'Two Takes')"); }
    ...
    await g.page("On the first day of the first month of the second year the Tent is finished. 'So Moses finished the work' — the words used of God at creation. The cloud covers the Tent of Meeting and the glory of the LORD fills it, and Moses CANNOT ENTER (Ex 40:34-35).\n\nA mobile Genesis 1-3 you carry through the desert (BEMA 23). And an unresolved question: how can a holy God live with a rebellious people?");
    g.setDate(2, 1, 1);
    await g.page("The next month is LEVITICUS — 'the owner's manual in the glove box of the tabernacle' (BEMA 25). Aaron is robed; fire comes out from the LORD and consumes the offering, and the people shout for joy and fall face down (Lev 9:24).\n\n'They weren't afraid. They had gotten to know this God in the desert well enough that they fall on their faces with JOY.' (BEMA 23)\n\nThen Nadab and Abihu bring their own fire, their own liturgy, and the same fire eats them (Lev 10). 'God's presence is good — it sustains life — and it is dangerous.' (BibleProject)");
    /* ---- CAMP ORDER ---- */
    await g.page("Numbers begins with a census: 603,550 men of fighting age. The camp is laid out like an Egyptian army camp, which Moses may have trained in: the Tent in the center, the Levites around it as a buffer, then the twelve tribes on four sides — JUDAH at the entrance, to the east.\n\nWhen the camp moves it 'unpeels like an orange' (Pawson, part 12), each tribe in order, each piece of furniture in order, the people a thousand paces from the ark.", { title: "THE CAMP" });
    ...
    if (place === 0) { g.trust(4); await g.page("Order in the camp is reverence made visible. 'Casualness doesn't have a place in God's camp.' (Pawson)"); }
