...
SINAI.STOPS.push({
  ...
  async run(g) {
    ...
    if (g.S.egyptFish) await g.page(`It is not only the rabble. Your own children remember the fish — ${g.S.egyptFish} of them, that you caught yourselves on the shore of the sea. The taste is still in their mouths. It is the manna they cannot taste any more.\n\n'We remember the fish' (Num 11:5). Nostalgia is a hunger that eats the present.`);
    ...
    if (c === 0) {
      const n = g.grumble("Kibroth-hattaavah"); g.flag("craved"); if (g.S.egyptFish) { g.trust(-Math.min(6, g.S.egyptFish)); g.S.H = Math.min(139, g.S.H + Math.min(10, g.S.egyptFish * 2)); }
      await g.page("'Nostalgia for Egypt is a lie your hunger tells you.'\n\nGRUMBLES: " + n + " of 10.");
    } else if (c === 1) { g.flock(-1); g.trust(-2); await g.page("A quiet meal. A smaller flock. A neighbor who saw the smoke."); }
    else { g.trust(5); await g.page("'Blessed are You, LORD our God, King of the universe, who brings forth bread from the earth' — or in this case, from the sky. Seventy blessings a day, the rabbis say."); }
...
SINAI.STOPS.push({
  ...
  async run(g) {
    ...
    const c = await g.choose("Your tribe is voting. 'They were thoroughly democratic — they took the majority verdict.' You stand:", [
    ...
    } else { await g.page("You watch them go up. You watch them come back. The ark did not move. 'It's too late, don't go up' — and then they still try."); }
...
SINAI.STOPS.push({
  ...
  async run(g) {
    await g.page("The cloud turns SOUTH. Back toward the Red Sea road.\n\n'It was not the terrain. It was the fact that God didn't move — He only moved a little at a time and stayed a very long time in each place, and they didn't dare move if God didn't.'\n\n'How would you like to be told: you are now redundant until you die?'\n\nThe years begin.", { title: "THE WANDERING" });
    /* ---- KORAH ---- */
    ...
    await g.generation();
    await g.page("The second census, taken at the end, will count 601,730 — almost exactly the first. 'Where God blesses, people multiply. The flat total shows God was NOT blessing them.'\n\nNot one adult from the first census remains, except two.\n\nThe fortieth year begins. The cloud moves north again, toward Kadesh. Toward the place it all went wrong.");
    g.setDate(40, 1, 1);
...
SINAI.STOPS.push({
  ...
  async run(g) {
    await g.page("Down into the ARABAH — the Rift Valley, 'the biggest crack in the earth's surface,' the Dead Sea at its bottom hundreds of feet below the ocean. The Bedouin call this stretch the Valley of the Scorpions. Dry. Dark. Snakes everywhere.\n\nDeuteronomy adds a detail: 'You are to pay the descendants of Esau in SILVER for the food you eat and the water you drink' (Deut 2:6). The desert has a market, and it is not friendly.", { title: "THE ARABAH" });
    ...
    {
      const c = await g.choose("A day south of Punon the Arabah opens onto the Gulf at EZION-GEBER (Num 33:35) — the second shore of your life. Salt wind, coral, and fish. It costs a day and a day of water.", ["Take the household down to the shore and fish", "Keep to the road"]);
      if (c === 0) {
        g.days(1); g.water(-1);
        const r = await g.minigame("fishing", { title: "EZION-GEBER", text: "The second shore. Move the hook to a fish; tap or SPACE to strike. ENTER when you are done." });
        g.food(r.food); g.S.egyptFish += r.caught;
        await g.page(r.caught ? `${r.caught} fish, ${r.food} lbs. Salt fish for the Arabah.\n\nThe old ones say it tastes exactly like Goshen. That is not a compliment to the manna.` : "The Gulf keeps its fish today. The manna is waiting in the morning, as always.");
      }
    }
  }
});
...
SINAI.STOPS.push({
  ...
  async run(g) {
    ...
    if (c < 2) {
      ...
      await g.page("The meal is a sacrifice. The wine is a libation. The music ends in the tents of the god. 'Idolatry leads to immorality' — or the other way round; it does not matter which door you came in.\n\nThe LORD's anger burns. A plague breaks out in the camp. TWENTY-FOUR THOUSAND die (Num 25:9)." + (who ? "\n\n" + who + " HAS DIED in the plague of Peor." : "\n\nYour household survives the plague. Barely."));
    } else {
